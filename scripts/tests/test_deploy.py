...
class DeployTest(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(prefix='map-deploy-test-')
...
'''
        ...
        self.env = dict(os.environ, PATH=str(self.root) + ':' + os.environ['PATH'],
                        CALL_LOG=str(self.log), ENV_FILE=str(REPO / '.env.docker.example'),
                        SSH_HOST='test-host', DEPLOY_DIR='apps/test-map',
                        CONTAINER_ENGINE='docker', DATASET_DIR='')
        self.dataset = self.root / 'dataset with spaces'
        self.dataset.mkdir()
        (self.dataset / 'dataset.json').write_text('{}')
        self.archive = self.root / 'image with spaces.tar'
        self.archive.touch()

    def run_deploy(self, *args, **env):
        return subprocess.run([str(REPO / 'scripts/deploy.sh'), *args],
